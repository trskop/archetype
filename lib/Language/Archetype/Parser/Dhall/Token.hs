...
module Language.Archetype.Parser.Dhall.Token (
    whitespace,
...
import           Language.Archetype.Parser.Dhall.Combinators
...
lineComment = do
    ...
    _ <- Language.Archetype.Parser.Dhall.Combinators.takeWhile predicate
...
blockCommentChunk =
  ...
  where
    characters = void (Language.Archetype.Parser.Dhall.Combinators.takeWhile1 predicate)
      where
        predicate c =
                '\x20' <= c && c <= '\x10FFFF' && c /= '-' && c /= '{'
            ||  c == '\n'
...
simpleLabel allowReserved = try (do
    ...
    rest <- Language.Archetype.Parser.Dhall.Combinators.takeWhile tailCharacter
